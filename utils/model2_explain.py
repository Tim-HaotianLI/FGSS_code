...
class CrossAttention(nn.Module):
    def __init__(self, feature_dim, num_heads=2, use_residual=True):
        ...
        self.dropout = nn.Dropout(0.4) # <==== 这个效果非常好
...
class BiCrossAttention(nn.Module):
    def __init__(self, feature_dim, num_heads=4, depth=3, dropout=0.1, use_pos_emb=False):
        ...
        self.debug_attn = {}  # 保存最近一次前向的注意力权重
        self.cache = {}

    def forward(self, f_global, f_object, bbox=None, return_attn=False):
        f_object = F.normalize(f_object, p=2, dim=1)
        f_global = F.normalize(f_global, p=2, dim=1)
        ...
        for l in self.layers:
            ...
            # 交叉注意力
            o_cross, attn_obj = l["cross_obj"](f_object, f_global, f_global)
            g_cross, attn_glb = l["cross_glb"](f_global, f_object, f_object)
            # 残差正则化
            ...
            if return_attn:
                # attn_* 形状：[B, num_heads, Q_len(=1), K_len(=1或patch数)]
                # print(f"attn_obj:{attn_obj} --- attn_glb:{attn_glb}")
                # print(f"o_cross shape: {o_cross.shape} --- g_cross shape: {g_cross.shape}")
                attn_obj_all.append(attn_obj)  # crop 查询 global 关键
                attn_glb_all.append(attn_glb)  # global 查询 crop 关键

        f_object = f_object.mean(dim=1)  # 相当于 f_object.unsqueeze(1)
        f_global = f_global.mean(dim=1)  # 相当于 f_global.unsqueeze(1)

        # ========== Step 3: gate 融合 ==========
        self.cache["f_object_pre"] = f_object.detach()
        self.cache["f_global_pre"] = f_global.detach()
        ...
        self.cache["f_fused_pre"] = fused.detach()
        self.cache["gate_vec"] = gate[0].detach()  # [D] 仅存一份向量
        self.cache["gate_scalar"] = gate.mean().detach()  # 方便打印
        ...
        if return_attn:
            # 仅保留最后一层的注意力更直观；也可 concat 所有层再平均
            self.debug_attn = {
                "obj_from_glb": attn_obj_all[-1],  # [B, H, 1, K]
                "glb_from_obj": attn_glb_all[-1],  # [B, H, 1, K]
                "o_class_shape": o_cross.shape
            }
...
class ObjectStateNet(nn.Module):
    ...
    def forward(self, img_global, img_object, labels=None, bbox=None, return_attn=True):
        ...
        if self.attn_type != "None":
            ...
            if self.attn_type == "bicross":
                f_object = self.attn(f_global = f_global,f_object = f_object,bbox = bbox,return_attn=return_attn)
