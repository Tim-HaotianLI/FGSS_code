...
class ObjectStateNet(nn.Module):
    def __init__(self,
        ...
        # ✅ 两个独立分类头
        self.classifier1 = CosFaceHead(classifier_in, num_classes[0])
        self.classifier2 = CosFaceHead(classifier_in, num_classes[1])

        # ✅ 两组可学习的类别偏置（bias）
        self.class_bias1 = nn.Parameter(torch.zeros(num_classes[0], self.feature_dim))
        self.class_bias2 = nn.Parameter(torch.zeros(num_classes[1], self.feature_dim))
    ...
    def forward(self, img_global, img_object, labels=None, bbox=None):
        ...
        if self.training and labels is not None:
            y1, y2 = labels  # labels 是 [tensor(...), tensor(...)]
            feat1 = feat + self.class_bias1[y1]
            feat2 = feat + self.class_bias2[y2]
            logits1 = self.classifier1(feat1)
            logits2 = self.classifier2(feat2)
        else:
            with torch.no_grad():
                proto1 = self.class_bias1
                proto2 = self.class_bias2
                sim1 = F.cosine_similarity(feat.unsqueeze(1), proto1.unsqueeze(0), dim=-1)
                sim2 = F.cosine_similarity(feat.unsqueeze(1), proto2.unsqueeze(0), dim=-1)
                idx1 = sim1.argmax(dim=1)
                idx2 = sim2.argmax(dim=1)
            feat1 = feat + self.class_bias1[idx1]
            feat2 = feat + self.class_bias2[idx2]
            logits1 = self.classifier1(feat1)
            logits2 = self.classifier2(feat2)
