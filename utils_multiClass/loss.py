...
class MultiTaskLoss(nn.Module):
    def __init__(self, weight1=None, weight2=None, gamma=1.5, smoothing=0.1,focal_weight=0.6, smooth_weight=0.4,
                 lambda1=1.0, lambda2=1.0):
        super().__init__()
        self.loss1 = FixedCombinedLoss(
            alpha=weight1,
            gamma=gamma,
            smoothing=smoothing,
    ...
    def forward(self, outputs, labels):
        """
        outputs: (out1, out2)
        labels: [label_task1, label_task2]
        """
        out1, out2 = outputs
        y1, y2 = labels
        loss1 = self.loss1(out1, y1)
        loss2 = self.loss2(out2, y2)
